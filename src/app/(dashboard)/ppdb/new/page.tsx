...
export default function PpdbNewPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (json.success) {
        Swal.fire("Berhasil", json.message, "success").then(() => router.push("/ppdb"));
      } else {
        ...
        <Card className="p-8 mb-6">
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1.25rem" }}>
            ...
            <div style={{ gridColumn: "span 2" }}>
              ...
              <textarea name="address" value={f.address} onChange={handleChange} rows={2} style={{ ...inputStyle, resize: "none" }} className="focus:border-sky-500 transition-colors" />
            </div>
