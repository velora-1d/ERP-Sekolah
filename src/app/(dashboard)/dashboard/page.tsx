...
async function getDashboardData() {
  const students = await prisma.student.findMany({ where: { deletedAt: null, status: "aktif" } });
  const employees = await prisma.employee.findMany({ where: { deletedAt: null, status: "aktif" } });
  const classrooms = await prisma.classroom.findMany({ where: { deletedAt: null } });
  const ppdb = await prisma.ppdbRegistration.findMany({ where: { deletedAt: null } });
  ...
  const ppdbPending = ppdb.filter((p: { status: string }) => p.status === "pending").length;
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
        ...
        />
        <KpiCard anim={5} label="Kepatuhan Lunas Infaq" value="0" color="#f59e0b" bg="#fef3c7" icon="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"
          footer={<><span>PA: <strong style={{ color: "#10b981" }}>0</strong></span><span>PI: <strong style={{ color: "#f59e0b" }}>0</strong></span></>}
        />
      </div>

      {/* KPI Cards Baris 2 */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
        <KpiCard anim={6} label="Pemasukan Bulan Ini" value={fmtRp(0)} color="#10b981" bg="#d1fae5" icon="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z" small
          footer={<><span>Data Real</span><strong style={{ color: "#10b981" }}>Bulan Ini</strong></>}
        />
        <KpiCard anim={7} label="Pengeluaran Bulan Ini" value={fmtRp(0)} color="#f43f5e" bg="#ffe4e6" icon="M17 9V7a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2m2 4h10a2 2 0 002-2v-6a2 2 0 00-2-2H9a2 2 0 00-2 2v6a2 2 0 002 2zm7-5a2 2 0 11-4 0 2 2 0 014 0z" small valueColor="#f43f5e"
          footer={<><span>Data Real</span><strong style={{ color: "#f43f5e" }}>Bulan Ini</strong></>}
        />
        <KpiCard anim={8} label="Total Saldo Tabungan" value={fmtRp(0)} color="#06b6d4" bg="#cffafe" icon="M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z" small
          footer={<><span>Seluruh Siswa</span><strong style={{ color: "#06b6d4" }}>Akumulatif</strong></>}
        />
        <KpiCard anim={9} label="Total Wakaf Masuk" value={fmtRp(0)} color="#d97706" bg="#fef3c7" icon="M12 8v13m0-13V6a2 2 0 112 2h-2zm0 0V5.5A2.5 2.5 0 109.5 8H12zm-7 4h14M5 12a2 2 0 110-4h14a2 2 0 110 4M5 12v7a2 2 0 002 2h10a2 2 0 002-2v-7" small
          footer={<><span>Penerimaan</span><strong style={{ color: "#d97706" }}>Akumulatif</strong></>}
        />
        <KpiCard anim={10} label="Siswa Menunggak" value="0" color="#f43f5e" bg="#ffe4e6" icon="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" valueColor="#f43f5e"
          footer={<><span>PA: <strong style={{ color: "#f43f5e" }}>0</strong></span><span>PI: <strong style={{ color: "#f43f5e" }}>0</strong></span></>}
        />
      </div>
    </>
  );
