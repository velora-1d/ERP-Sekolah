...
export default function TeachersPage() {
  ...
  async function loadData(q = "", stat = "") {
    setLoading(true);
    try {
      const res = await fetch(`/api/teachers?q=${encodeURIComponent(q)}&status=${stat}`);
          ...
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan={5} style={{ padding: "4rem 2rem", textAlign: "center" }}>Belum Ada Data Guru</td></tr>
              ) : (
                data.map((t: any, i) => {
