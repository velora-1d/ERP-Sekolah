...
export default function StudentsPage() {
  ...
  const catColors: Record<string, string[]> = { reguler: ["#f1f5f9", "#475569"], yatim: ["#fef3c7", "#92400e"], kurang_mampu: ["#ede9fe", "#6b21a8"] };
  const statusColors: Record<string, string[]> = { aktif: ["#d1fae5", "#047857"], lulus: ["#cffafe", "#0e7490"], pindah: ["#ffedd5", "#c2410c"], nonaktif: ["#e5e7eb", "#6b7280"] };
        ...
        <div style={{ padding: "2rem", position: "relative", zIndex: 10 }}>
          <div className="flex flex-wrap items-center justify-between gap-4">
            ...
            <div className="flex gap-2 items-center flex-wrap">
              <input type="text" value={search} onChange={handleSearch} placeholder="Cari nama/NISN..." 
                style={{ padding: "0.5rem 0.75rem", background: "rgba(255,255,255,0.2)", backdropFilter: "blur(10px)", color: "#fff", border: "1.5px solid rgba(255,255,255,0.3)", borderRadius: "0.625rem", fontSize: "0.8125rem", width: 180, outline: "none" }} />
      ...
      <div style={{ background: "#fff", borderRadius: "1rem", border: "1px solid #e2e8f0", overflow: "hidden" }}>
        ...
        <div style={{ overflowX: "auto" }}>
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            ...
            <tbody>
              {loading ? (
                <tr><td colSpan={6} style={{ padding: "3rem 2rem", textAlign: "center", fontSize: "0.8125rem", color: "#94a3b8", fontStyle: "italic" }}>Memuat data siswa...</td></tr>
              ) : data.length === 0 ? (
                <tr><td colSpan={6} style={{ padding: "4rem 2rem", textAlign: "center" }}>
                  <div style={{ display: "flex", flexDirection: "column", alignItems: "center" }}>
                    ...
                    <p style={{ fontSize: "0.8125rem", color: "#94a3b8", marginTop: "0.375rem" }}>Data tidak ditemukan.</p>
                  </div>
                </td></tr>
              ) : (
                data.map((s: any, i) => {
                  ...
                  const catLabel = (s.category || "reguler").replace("_", " ");
                  ...
                  return (
                    <tr key={s.id} className="hover:bg-slate-50 transition-colors" style={{ borderBottom: "1px solid #f1f5f9" }}>
                      ...
                      <td style={{ padding: "1rem 1.5rem" }}>
                        <span style={{ display: "inline-flex", padding: "0.25rem 0.625rem", fontSize: "0.6875rem", fontWeight: 600, borderRadius: 999, background: cc[0], color: cc[1], textTransform: "capitalize" }}>{catLabel}</span>
                      </td>
                      <td style={{ padding: "1rem 1.5rem" }}>
                        {s.classroom ? (
